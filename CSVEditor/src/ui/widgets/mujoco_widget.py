...
class MuJoCoWidget(QOpenGLWidget):
    ...
    def _update_camera_target(self):
        """Keep the free camera focused slightly below the root position."""
        if self.data is None:
            return
        root_pos = np.array(self.data.qpos[0:3], dtype=float)
        self.cam.lookat = root_pos + np.array([0.0, 0.0, -0.4], dtype=float)
