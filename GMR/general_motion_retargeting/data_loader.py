...
def load_robot_motion(motion_file):
    ...
    with open(motion_file, "rb") as f:
        ...
        motion_root_pos = np.asarray(motion_data["root_pos"], dtype=np.float32)
        motion_root_rot_xyzw = np.asarray(motion_data["root_rot"], dtype=np.float32)
        motion_root_rot = motion_root_rot_xyzw[:, [3, 0, 1, 2]]  # from xyzw to wxyz
        motion_dof_pos = np.asarray(motion_data["dof_pos"], dtype=np.float32)
