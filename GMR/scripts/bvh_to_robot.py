...
def resample_bvh_data(bvh_data_frames, src_fps, tgt_fps):
    ...
    for t in tgt_times:
        ...
        for joint_name in frame_prev.keys():
            ...
            slerp = Slerp([0.0, 1.0], R.concatenate([rot_prev_r, rot_next_r]))
            rot_interp = slerp([alpha]).as_quat(scalar_first=True)[0]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--robot",
        choices=["unitree_g1", "unitree_g1_auto", "unitree_g1_with_hands", "booster_t1", "stanford_toddy", "fourier_n1", "engineai_pm01", "pal_talos", "roboot16"],
        default="unitree_g1",
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Run retargeting and save output without opening the MuJoCo viewer.",
    )
